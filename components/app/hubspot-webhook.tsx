...
function buildSnippet(url: string, token: string): string {
  return [
    ...
    "    // O embed dispara onFormSubmit E onFormSubmitted no mesmo envio: evita o",
    "    // POST duplicado mandando o mesmo conteudo so uma vez por janela curta.",
    "    var key = JSON.stringify(fields), now = Date.now();",
